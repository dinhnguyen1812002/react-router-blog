...
interface OAuthButtonsProps {
	...
	className?: string;
	/** Full-width stacked buttons with labels, or compact icon-only grid */
	layout?: "stack" | "icon-grid";
}
		...
		return (
			<div className={cn(className)}>
				<div
					className={cn(
						"animate-pulse",
						isGrid ? "grid grid-cols-3 gap-3" : "flex flex-col gap-3",
					)}
				>
					{[1, 2, 3].map((i) => (
						<div
			...
			>
				{providers.map((provider) => (
					<Button
						...
						className={cn(
							"w-full font-normal text-foreground shadow-none",
							"hover:bg-muted/40 dark:hover:bg-muted/25",
							isGrid
								? "h-11 min-w-0 shrink-0 rounded-md p-0 [&_svg]:h-6 [&_svg]:w-6"
								: "justify-center gap-3 rounded-md py-3",
						)}
					>
						{loadingProvider === provider.id ? (
							<Loader2
								className={cn("animate-spin", isGrid ? "h-6 w-6" : "h-5 w-5")}
							/>
						) : (
							provider.icon
						)}
