...
export function ListView({
	posts,
	onDelete,
	formatDateSimple: formatDate = formatDateSimple,
}: PostListProps) {
	return (
		<div className="space-y-2 ">
			{posts.map((post) => (
				<div
					key={post.id}
					className="group overflow-hidden hover:bg-muted/50 transition-all duration-300 cursor-pointer rounded-lg"
				>
					<div className="flex flex-col md:flex-row gap-3 p-3 border-b-2 border-gray-200 dark:border-gray-700">
						<Link
							to={`/articles/${post.slug}`}
							className="flex-1 flex md:flex-row gap-3"
						>
                    ...
                    >
                      {post.is_publish ? "Published" : "Draft"}
						...
						<div className="flex md:flex-col items-center gap-2 justify-end">
							<Link
								to={`/dashboard/article/${post.id}/edit`}
