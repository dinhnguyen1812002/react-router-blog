import { Clock, FileText, Image, Zap } from "lucide-react";
...
export function LoadingPerformanceIndicator({
	...
	useEffect(() => {
		...
		const updateMetrics = () => {
			...
			const imagesLoaded = imageEntries.filter(
			...
			const avgImageLoadTime =
				imageLoadTimes.length > 0
					? imageLoadTimes.reduce((sum, time) => sum + time, 0) /
			...
			<div className="bg-white dark:bg-black border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg p-4 animate-in slide-in-from-bottom-2 duration-300">
				...
				<div className="space-y-3">
					...
					<div className="flex items-center justify-between">
						...
						<span
							className={`text-sm font-medium px-2 py-1 rounded ${bgColor} ${color}`}
						>
							{status}
						</span>
					</div>
					...
					{metrics.totalImages > 0 && (
						<div className="mt-3">
							<div className="flex justify-between text-xs text-gray-600 dark:text-gray-400 mb-1">
								...
								<span>
									{Math.round(
										(metrics.imagesLoaded / metrics.totalImages) * 100,
									)}
									%
								</span>
							</div>
							<div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5">
								<div
									...
									style={{
										width: `${(metrics.imagesLoaded / metrics.totalImages) * 100}%`,
									}}
								/>
							</div>
						</div>
...
export function useLoadingPerformance() {
	...
	useEffect(() => {
		...
		const showPerf =
			localStorage.getItem("show-performance-indicator") === "true";
