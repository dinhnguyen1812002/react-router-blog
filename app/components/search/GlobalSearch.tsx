...
import { searchApi, type GlobalSearchItem, type GlobalSearchResponse } from "~/api/search";
...
function normalizeResponse(data: GlobalSearchResponse | undefined): GlobalSearchResponse {
  return {
    posts: data?.posts ?? [],
    series: data?.series ?? [],
    users: data?.users ?? [],
...
export function GlobalSearch({
  ...
  const moveActive = (delta: number) => {
    ...
    setActiveIndex((prev) => {
      const next = prev < 0 ? (delta > 0 ? 0 : flattened.length - 1) : prev + delta;
      ...
      <div className="py-2">
        ...
        <div className="space-y-1">
          {items.map((it) => {
            ...
            const flatIndex = flattened.findIndex((f) => f.kind === kind && f.item.slug === it.slug);
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-xs text-muted-foreground truncate">{it.authorName}</div>
                </div>
      ...
      <div className="flex items-center gap-2 bg-muted rounded-lg px-3 py-2">
        ...
        <input
          ...
          className={[
            ...
            .join(" ")}
        />
        {isFetching ? <Loader2 className="w-4 h-4 animate-spin text-muted-foreground" /> : null}
      </div>
          ...
          {!enabled ? null : (
            <div className="max-h-[70vh] overflow-auto">
              ...
              {isFetching && !hasAnyResults ? (
                <div className="px-3 py-3 text-sm text-muted-foreground">Đang tìm kiếm…</div>
              ) : null}
