...
export function meta({}: Route.MetaArgs) {
  return [
    ...
    { name: "description", content: "Khám phá danh sách các bài viết mới nhất, sâu sắc và thú vị về đa dạng chủ đề trên Inkwell." },
    { property: "og:title", content: "Bài viết - Khám phá nội dung | Inkwell" },
    { property: "og:description", content: "Khám phá danh sách các bài viết mới nhất, sâu sắc và thú vị về đa dạng chủ đề trên Inkwell." },
  ];
}
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  const sortBy = (url.searchParams.get('sortBy') || 'newest') as 'newest' | 'views';
...
export default function ArticlesIndex({ loaderData }: Route.ComponentProps) {
  const { posts, currentPage, sortBy, categorySlug, tagSlug, searchQuery } = loaderData;
              ...
              {searchQuery && (
                <div className="mb-6 pb-4 border-b border-black dark:border-white">
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    Kết quả tìm kiếm cho "<span className="font-medium text-black dark:text-white">{searchQuery}</span>"
                  </p>
