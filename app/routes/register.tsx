...
import { User, Mail, Lock, UserPlus, Eye, EyeOff, CheckCircle2, ChevronsRight } from "lucide-react";
...
export default function RegisterPage() {
  ...
  const passwordStrength = {
    strength: Math.round(((hasLower ? 1 : 0) + (hasUpper ? 1 : 0) + (hasDigit ? 1 : 0) + (hasSpecial ? 1 : 0) + (noSpace ? 1 : 0) + (minLen ? 1 : 0)) * 100 / 6),
    label: passwordValue.length === 0 ? '' : passwordValue.length < 6 ? 'Weak' : 
           ((hasLower ? 1 : 0) + (hasUpper ? 1 : 0) + (hasDigit ? 1 : 0) + (hasSpecial ? 1 : 0) + (noSpace ? 1 : 0)) >= 4 ? 'Strong' : 'Medium',
    color: passwordValue.length === 0 ? 'bg-gray-300' : 
           passwordValue.length < 6 ? 'bg-red-500' :
           ((hasLower ? 1 : 0) + (hasUpper ? 1 : 0) + (hasDigit ? 1 : 0) + (hasSpecial ? 1 : 0) + (noSpace ? 1 : 0)) >= 4 ? 'bg-green-500' : 'bg-yellow-500'
  };
          ...
          <div className="space-y-6">
            <div className="flex items-start space-x-4">
              ...
              <div>
                ...
                <p className="text-slate-300 text-sm">Quick and easy registration process</p>
              </div>
            </div>
            <div className="flex items-start space-x-4">
              <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-white/20 backdrop-blur-sm flex items-center justify-center">
              ...
              <div>
                ...
                <p className="text-slate-300 text-sm">Customize your dashboard to fit your needs</p>
              </div>
            </div>
            <div className="flex items-start space-x-4">
              <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-white/20 backdrop-blur-sm 
              flex items-center justify-center">
                <span className="text-xl font-bold">3</span>
              </div>
              <div>
                <h3 className="font-semibold text-white mb-1">Start collaborating</h3>
                <p className="text-slate-300 text-sm">Invite your team and begin working together</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-8">
          <div className="relative">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-4 bg-slate-50 lg:bg-white text-slate-500">Already have an account?</span>
            </div>
          </div>
          <button
            type="button"
