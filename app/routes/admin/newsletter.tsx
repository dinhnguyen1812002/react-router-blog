...
function StatusBadge({
  ...
  status: "PENDING" | "ACTIVE" | "UNSUBSCRIBED" | "BOUNCED" | "COMPLAINED" | "SUSPENDED"
}) {
  if (status === "PENDING") {
    return (
...
function CampaignStatusBadge({ status }: { status: string }) {
  ...
  if (status === "SCHEDULED") {
    return <Badge variant="outline" className={`${common} text-amber-600 border-amber-200`}>{status}</Badge>
  }

  if (status === "SENDING") {
    return <Badge className={`${common} bg-blue-100 text-blue-700 border-blue-200`}>{status}</Badge>
  }

  if (status === "SENT") {
    return <Badge className={`${common} bg-green-100 text-green-700 border-green-200`}>{status}</Badge>
  }

  return <Badge variant="destructive" className={common}>{status}</Badge>
}
              ...
              data.content.map((subscriber) => (
                <tr key={subscriber.id}>
                  ...
                  <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">
                    {subscriber.createdAt ? formatDateSimple(subscriber.createdAt) : "—"}
                  </td>
                  <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">
...
export default function AdminNewsletterPage() {
  ...
  const createCampaignMutation = useMutation({
    mutationFn: async () => {
      const payload = {
        ...
        ...(createCampaignForm.textContent.trim()
          ...
          : {}),
        ...(createCampaignForm.fromName.trim() ? { fromName: createCampaignForm.fromName.trim() } : {}),
        ...(createCampaignForm.fromEmail.trim() ? { fromEmail: createCampaignForm.fromEmail.trim() } : {}),
        ...(createCampaignForm.replyTo.trim() ? { replyTo: createCampaignForm.replyTo.trim() } : {}),
        ...(createCampaignForm.scheduledAt.trim()
          ? { scheduledAt: new Date(createCampaignForm.scheduledAt).toISOString() }
          : {}),
        ...
        ...(createCampaignForm.utmSource.trim() ? { utmSource: createCampaignForm.utmSource.trim() } : {}),
        ...(createCampaignForm.utmMedium.trim() ? { utmMedium: createCampaignForm.utmMedium.trim() } : {}),
        ...(createCampaignForm.utmCampaign.trim() ? { utmCampaign: createCampaignForm.utmCampaign.trim() } : {}),
      };
      ...
      if (!payload.htmlContent.trim()) throw new Error("Vui lòng nhập htmlContent campaign");
    ...
    onError: (error: any) => {
      toast.error(error?.response?.data?.message || error?.message || "Tạo campaign thất bại");
    },
  });
  ...
  const sendCampaignMutation = useMutation({
    ...
    onError: (error: any) => {
      toast.error(error?.response?.data?.message || error?.message || "Gửi campaign thất bại");
    },
  });
          ...
          <div>
            ...
            <p className="text-2xl font-semibold text-gray-900 dark:text-white">
              {allLoading
                ? "—"
                : formatNumber(
                  (allSubscribers?.content || []).filter((s) => s.status === "PENDING").length
                )}
            </p>
          </div>
        </div>
      ...
      <div className="space-y-4">
        ...
        <div className="bg-white dark:bg-black rounded-lg shadow-sm border border-gray-100 dark:border-gray-800 p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              <label className="text-sm text-gray-700 dark:text-gray-200">Tên</label>
              <input
                value={createCampaignForm.name}
                onChange={(e) => setCreateCampaignForm((p) => ({ ...p, name: e.target.value }))}
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setCreateCampaignForm((p) => ({ ...p, subject: e.target.value }))}
                className="mt-1 w-full rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-black px-3 py-2 text-sm"
          ...
          <div className="mt-3">
            <label className="text-sm text-gray-700 dark:text-gray-200">HTML content</label>
            <textarea
              value={createCampaignForm.htmlContent}
              onChange={(e) => setCreateCampaignForm((p) => ({ ...p, htmlContent: e.target.value }))}
          ...
          <div className="mt-3">
            <label className="text-sm text-gray-700 dark:text-gray-200">Text content (optional)</label>
            <textarea
              value={createCampaignForm.textContent}
              onChange={(e) => setCreateCampaignForm((p) => ({ ...p, textContent: e.target.value }))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
            <div>
              <label className="text-sm text-gray-700 dark:text-gray-200">ScheduledAt (optional)</label>
              <input
                type="datetime-local"
                value={createCampaignForm.scheduledAt}
                onChange={(e) => setCreateCampaignForm((p) => ({ ...p, scheduledAt: e.target.value }))}
                className="mt-1 w-full rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-black px-3 py-2 text-sm"
              />
            ...
            <div className="space-y-2">
              <div>
                <label className="text-sm text-gray-700 dark:text-gray-200">From name (optional)</label>
                <input
                  value={createCampaignForm.fromName}
                  onChange={(e) => setCreateCampaignForm((p) => ({ ...p, fromName: e.target.value }))}
                  className="mt-1 w-full rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-black px-3 py-2 text-sm"
                />
              </div>
              <div>
                <label className="text-sm text-gray-700 dark:text-gray-200">From email (optional)</label>
                <input
                  value={createCampaignForm.fromEmail}
                  onChange={(e) => setCreateCampaignForm((p) => ({ ...p, fromEmail: e.target.value }))}
                  className="mt-1 w-full rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-black px-3 py-2 text-sm"
                />
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-3">
            <div>
              <label className="text-sm text-gray-700 dark:text-gray-200">Target segment</label>
              <input
                value={createCampaignForm.targetSegment}
                onChange={(e) => setCreateCampaignForm((p) => ({ ...p, targetSegment: e.target.value }))}
                className="mt-1 w-full rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-black px-3 py-2 text-sm"
                placeholder="all"
              />
            </div>
            <div>
              <label className="text-sm text-gray-700 dark:text-gray-200">Target tags (comma separated)</label>
              <input
                value={createCampaignForm.targetTags}
                onChange={(e) => setCreateCampaignForm((p) => ({ ...p, targetTags: e.target.value }))}
                className="mt-1 w-full rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-black px-3 py-2 text-sm"
                placeholder="vip,regular"
              />
            </div>
            <div>
              <label className="text-sm text-gray-700 dark:text-gray-200">Batch size</label>
              <input
                type="number"
                value={createCampaignForm.batchSize}
                onChange={(e) => setCreateCampaignForm((p) => ({ ...p, batchSize: Number(e.target.value) }))}
                className="mt-1 w-full rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-black px-3 py-2 text-sm"
              />
            </div>
          </div>
          ...
          <p className="mt-3 text-xs text-gray-500 dark:text-gray-400">
            Variables: <code>{"{{firstName}}"}</code>, <code>{"{{email}}"}</code>, <code>{"{{unsubscribeUrl}}"}</code>
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-black rounded-lg shadow-sm border border-gray-100 dark:border-gray-800 p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="text-sm text-gray-500 dark:text-gray-400">
              {campaignsLoading ? "—" : formatNumber(campaignsData?.totalElements ?? 0)} campaigns
            </div>
          </div>
                  ...
                  campaignsData.content.map((c) => {
                    ...
                    return (
                      <tr key={c.id}>
                        ...
                        <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">
                          {c.scheduledAt ? formatDateSimple(c.scheduledAt) : "—"}
                        </td>
                        <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">
