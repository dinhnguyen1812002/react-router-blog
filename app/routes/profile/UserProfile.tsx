...
export const meta: MetaFunction<typeof loader> = ({ data, params }) => {
	...
	if (!username || !profile) {
		return [
			{ title: "Hồ sơ người dùng - Inkwell" },
			{
				name: "description",
				content:
					"Khám phá hồ sơ người dùng trên Inkwell - nền tảng blog cao cấp.",
			},
		];
	}
	...
	const profileImage =
		profile.avatar || `https://inkwell.vn/api/og/profile/${username}`;
	...
	return [
		...
		{ property: "og:image:height", content: "630" },
		{
			...
			content: `Hồ sơ của ${displayName} trên Inkwell`,
		},
			...
			? [
					{
						...
						content: profile.username.split(" ").slice(1).join(" "),
					},
				]
			: []),
			...
			? [
					{
						...
						content: `@${profile.socialMediaLinks.TWITTER}`,
					},
				]
			: []),
							...
							{profile?.customInformation ? (
								...
								<div className="bg-card rounded-xl p-6 md:p-8 shadow-soft">
									...
									<div className="prose prose-lg text-muted-foreground space-y-4">
										<p>
											Xin chào! Tôi là {profile?.username || "một tác giả"}, đam
											mê chia sẻ kiến thức và kinh nghiệm qua blog.
										</p>
										{profile?.bio && (
										...
										<p>
											Tham gia Inkwell để khám phá thêm nhiều bài viết và kết
											nối với cộng đồng tác giả.
										</p>
									</div>
								</div>
							)}
