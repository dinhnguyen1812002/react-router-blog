...
export const profileApi = {
    ...
    updateProfileMarkdown: async (markdownContent: string) => {
        const response = await apiClient.put<UserProfileResponse>('/user/profile', {
            markdownContent
        });
    ...
    getProfileByUsername: async (username: string) => {
        const response = await apiClient.get<UserProfileResponse>(`/user/profile/${username}`);
        return response.data;
    },
    ...
    patchProfile: async (data: Partial<UpdateProfileRequest>) => {
        const response = await apiClient.patch<UserProfileResponse>('/profile', data);
        return response.data;
    },
