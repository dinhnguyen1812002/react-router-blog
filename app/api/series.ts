...
export const seriesApi = {
	...
	getSeriesByUser: async (
		...
		const response = await apiClient.get(
			`/series/user/${userId}?${params.toString()}`,
		);
		return response.data;
	},

	searchSeries: async (
		searchData: SeriesSearchRequest,
	): Promise<ApiResponse<PaginatedResponse<Series>>> => {
	...
	getPopularSeries: async (
		...
		const response = await apiClient.get(
			`/series/popular?${params.toString()}`,
		);
		return response.data;
	},

	addPostToSeries: async (
		seriesId: string,
	...
	reorderSeriesPosts: async (
		seriesId: string,
		data: ReorderSeriesPostsRequest,
	): Promise<ApiResponse<Series>> => {
		const response = await apiClient.put(
			`/series/${seriesId}/posts/reorder`,
			data,
		);
		return response.data;
	},
